...
2. **`lighthouse` — `NO_FCP` real, causa distinta al hallazgo de WSL:** dos intentos de fix fallaron primero (`startServerReadyTimeout: 60000` y cambiar `npm run preview` por `npx vite preview` directo) — ambos descartados con evidencia: el log mostraba que el timeout de 60s se cumplia exacto y aun asi Lighthouse esperaba sus 30s internos de FCP sin pintar nada. Se clono el repo real y se reprodujo el build+preview fuera de CI para descartar teorias a ciegas: `curl` contra `http://127.0.0.1:4173/` confirmo HTML SSR completo y valido (titulo, contenido real, ~46KB de body), y los 7 assets criticos (CSS, JS, imagenes preloaded) responden 200 — **el server y el build estan bien, no es un bug de la app**. Causa real (documentada oficialmente en `googlechrome.github.io/lighthouse-ci/docs/troubleshooting.html` y multiples issues de `GoogleChrome/lighthouse-ci` — #398, #795, #766, mismo patron `NO_FCP` en GitHub Actions): Chrome headless necesita `--no-sandbox` explicito en runners de CI: sin el, el proceso de sandboxing de Chrome puede fallar/colgarse silenciosamente durante la navegacion en vez de dar un error claro. Fix: `frontend2.0/.lighthouserc.json` -> `collect.settings.chromeFlags: "--no-sandbox --disable-gpu --disable-dev-shm-usage"`. `startServerReadyTimeout: 60000` y `npx vite preview` directo se mantienen (no hacian dano, solo no eran la causa raiz).
