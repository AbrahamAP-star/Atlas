...
async function uploadFile(file: File, token: string): Promise<string> {
  ...
  if (!res.ok)
    throw new Error(
      await readErrorMessage(res, "Could not upload the file to IPFS."),
    );
  ...
  if (!res.ok)
    throw new Error(
      await readErrorMessage(
        res,
        "Could not upload the project information to IPFS.",
      ),
    );
  const data = await res.json();
  return data.cid as string;
...
export function usePinataUpload() {
  ...
  async function upload(
    ...
    try {
      ...
      const documentCID = document
        ? await uploadFile(document, token)
        : undefined;
      return await uploadMetadata(
        { title, description, imageCID, documentCID },
        token,
      );
    } catch (err) {
