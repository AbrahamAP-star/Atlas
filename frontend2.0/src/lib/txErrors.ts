...
export const errorMessages: Record<string, string> = {
  ...
  ProjectClosed:
    "The project was already withdrawn by its creator, it no longer accepts pledges.",
  ProjectNotSuccessful:
    "The project hasn't reached the minimum goal for withdrawal yet.",
  ...
  ProjectHasActiveFunds:
    "You can't delete this project: it still has unclaimed pledges. Backers must refund themselves first, or you must claim the funds.",
  TransferFailed: "The transfer failed, please try again.",
  ReentrancyGuardReentrantCall:
    "Operation blocked for security reasons, please try again.",
...
export function extractErrorName(callError: unknown): string | undefined {
  ...
  const raw = (
    callError.walk((e) => "data" in (e as { data?: unknown })) as {
      data?: unknown;
    } | null
  )?.data;
