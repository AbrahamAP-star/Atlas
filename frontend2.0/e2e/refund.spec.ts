...
test("pledge → refund, balance goes back up", async ({
  creatorPage,
  backerPage,
  request,
}) => {
  ...
  await creatorDemo
    ...
    .fill("Never reaches its goal, on purpose.");
  ...
  await expect(
    creatorDemo.locator(".project-card", { hasText: PROJECT_TITLE }),
  ).toBeVisible({
    timeout: 30_000,
  });
  ...
  const balanceBeforePledge = await getBalanceWei(
    request,
    ANVIL_ACCOUNTS.backer,
  );
  ...
  const refundButton = backerDemo.getByRole("button", {
    name: /Request refund/,
  });
  await expect(refundButton).toBeVisible({ timeout: 30_000 });

  const balanceAfterPledge = await getBalanceWei(
    request,
    ANVIL_ACCOUNTS.backer,
  );
  ...
  const balanceAfterRefund = await getBalanceWei(
    request,
    ANVIL_ACCOUNTS.backer,
  );
