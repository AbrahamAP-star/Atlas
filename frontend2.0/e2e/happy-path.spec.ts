...
test("create → pledge → claim", async ({ creatorPage, backerPage }) => {
  ...
  await expect(creatorDemo.getByText(/0xf39F.*2266/)).toBeVisible({
    timeout: 15_000,
  });
  ...
  const projectCard = creatorDemo.locator(".project-card", {
  ...
  await expect(backerDemo.getByText(/0x7099.*79C8/)).toBeVisible({
    timeout: 15_000,
  });
  ...
  await expect(
    backerDemo.getByText(`Raised: ${GOAL_ETH} ETH of ${GOAL_ETH} ETH`),
  ).toBeVisible({
    timeout: 30_000,
  });

  // --- Creator: reload to pick up the now-successful project, then claim ---
  // gotoAppReady, not a bare reload(): reload() only waits for the "load"
  // event, which fires on pre-hydration SSR markup — the click right after
  // it can land on inert HTML and do nothing (same gap gotoAppReady closes
  // for the very first navigation, see its comment above).
  await gotoAppReady(creatorPage);
  await creatorDemo.scrollIntoViewIfNeeded();
  await creatorDemo.getByRole("button", { name: "Explore projects" }).click();
  await creatorDemo
    .locator(".project-card", { hasText: PROJECT_TITLE })
    .click();
